        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        {% for order in orders %}
                        <tr class="{% if order.is_locked %}table-secondary{% endif %}">
                            <td><small>{{ order.order_no }}</small></td>
                            <td>{% if order.user %}{{ order.user.nickname|default:order.user.username }}{% else %}-{% endif %}</td>
...
function rejectOrder(orderId) {
    ...
    form.method = "POST";
    form.action = "/orders/admin/" + orderId + "/confirm/";
    form.innerHTML = '<input name="action" value="reject"><input name="reject_reason" value="' + reason + '">';
    document.body.appendChild(form);
    form.submit();
}
...
function rescheduleOrder(orderId) {
    ...
    const form = document.createElement("form");
    form.method = "POST";
    form.action = "/orders/admin/" + orderId + "/reschedule/";
    form.innerHTML = '<input name="new_date" value="' + newDate + '"><input name="new_time" value="' + newTime + '">';
    document.body.appendChild(form);
    form.submit();
}
...
function delayOrder(orderId) {
    ...
    form.innerHTML = '<input name="delay_minutes" value="' + minutes + '"><input name="delay_reason" value="' + (reason || "") + '">';
    document.body.appendChild(form);
    form.submit();
}

let assignTechOrderId = null;
function assignTech(orderId) {
    assignTechOrderId = orderId;
...
document.getElementById("assignTechForm").addEventListener("submit", function(e) {
    ...
    form.innerHTML = '<input name="technician_id" value="' + techId + '">';
    document.body.appendChild(form);
    form.submit();
});
